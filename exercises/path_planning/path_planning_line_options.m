...
function path_planning_line_options
close all
...
% PLEASE UNCOMMENT ONE OF THE FOLLOWING TWO LINES
robot = load_robot('ABB', 'IRB52')
q = [0 0 0 0 0 0];
%robot = load_robot('RETHINK', 'SAWYER')
%q = [0 0 0 0 0 0 0];

% PLEASE SELECT THE SOLUTION
% option = 'vanilla'
% option = 'moore'
% this is the more general solution that works with redundant robots and is 
% also damped (restricts the maximum speeds on a singularity)
option = 'dampedmoore' 
...
while 1
   ...
   % iJ is the inverse Jacobian computed according to option.
   iJ = inverse_jacobian(J, option);
   qd1 = iJ*xd;
   q = q + delta_time*qd1;
   ...
   % Try to move following a vector that moves you next to the line
   qd2 = iJ*[error_line_vector' 0 0 0]';
