function q = inversekinematic_prosixc4l(robot, T)
...
for i=1:2:size(q,2),
    ...
    qtemp = solve_spherical_wrist2(robot, q(:,i), T, 1,'algebraic'); %Muñeca arriba
    qtemp(4:6)=normalize(qtemp(4:6));
    q(:,i)=qtemp;
    
    qtemp = solve_spherical_wrist2(robot, q(:,i), T, -1, 'algebraic'); %Muñeca abajo
    qtemp(4:6)=normalize(qtemp(4:6));
    q(:,i+1)=qtemp;
end
...
if ~isreal(gamma)
    disp('WARNING:inversekinematic_prosixc4l: el punto no es alcanzable para esta solución, soluciones imaginarias'); 
end
