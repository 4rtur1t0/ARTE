%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Q = INVERSEKINEMATIC_IRB1660ID(robot, T)	
%   Solves the inverse kinematic problem for the ABB IRB 1660ID robot
%   where:
%   robot stores the robot parameters.
%   T is an homogeneous transform that specifies the position/orientation
%   of the end effector.
%
%   A call to Q=INVERSEKINEMATIC_IRB1660ID returns 8 possible solutions, thus,
%   Q is a 6x8 matrix where each column stores 6 feasible joint values.
%
%   
%   Example code:
%
%   abb=load_robot('abb', 'IRB1660ID');
%   q = [0 0 0 0 0 0];	
...
function q = inversekinematic_irb1660id(robot, T)
...
%brazo ficticio que nos quita el desfase
l3 = sqrt (A2^2 + L3^2);
...
if ~isreal(gamma)
    disp('WARNING:inversekinematic_irb1660id: the point is not reachable for this configuration, imaginary solutions'); 
    %gamma = real(gamma);
end
...
if ~isreal(eta)
   disp('WARNING:inversekinematic_irb1660: the point is not reachable for this configuration, imaginary solutions'); 
   %eta = real(eta);
end
