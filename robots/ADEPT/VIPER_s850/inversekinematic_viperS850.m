...
function q = inversekinematic_viperS850(robot, T)
...
L2=a(2);
L3_0=d(4);
L3 = sqrt(L3_0^2+a(3)^2); %codo sistemas 2-3-4
...
if ~isreal(gamma)
    disp('WARNING:inversekinematic_vipers850: the point is not reachable for this configuration, imaginary solutions'); 
    %gamma = real(gamma);
end
...
q3(1) = pi/2 + lambda - eta;
q3(2) = eta + lambda - 3*pi/2;
