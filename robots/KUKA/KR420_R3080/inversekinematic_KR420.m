%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Q = INVERSEKINEMATIC_KR420(robot, T)	
%   Solves the inverse kinematic problem for the KUKA KR420 R3080 robot
%   where:
%   robot stores the robot parameters.
%   T is an homogeneous transform that specifies the position/orientation
%   of the end effector.
%
%   A call to Q=INVERSEKINEMATIC_KR420 returns 8 possible solutions, thus,
%   Q is a 6x8 matrix where each column stores 6 feasible joint values.
%
%   
%   Example code:
%
%   >>abb=load_robot('KUKA', 'KR420_R3080');
%   >>q = [0 0 0 0 0 0];	
%   >>T = directkinematic(abb, q);
%
%   %Call the inversekinematic for this robot
%
%   >> qinv = inversekinemaqtic(abb, T);
%
...
function q = inversekinematic_KR420(robot, T)
...
if ~isreal(eta)
   disp('WARNING:inversekinematic_KR_420: the point is not reachable for this configuration, imaginary solutions'); 
   %eta = real(eta);
end
