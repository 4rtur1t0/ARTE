...
%   KUKA KR420 R3080.
%
%   Authores: Alicia Espigares Erades
%             Adrián Rodriguez Moreno
%             Arturo Gil
%
%   Universidad Miguel Hernandez de Elche. 
%   email: arturo.gil@umh.es  date: 07/01/2019
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
robot.name= 'KR420_R3080';

robot.DH.theta= '[q(1) q(2)-pi/2 q(3) q(4) q(5) q(6)+pi]';
robot.DH.d='[0.1045 0 0 0.1275 0 0.0290]';
robot.DH.a='[0.0500 0.1300 0 0 0 0]';
robot.DH.alpha= '[-pi/2 0 -pi/2 pi/2 -pi/2 0]';
robot.J=[];


robot.inversekinematic_fn = 'inversekinematic_KR420(robot, T)';
...
robot.maxangle =[deg2rad(-185) deg2rad(185); %Axis 1, minimum, maximum
                deg2rad(-130) deg2rad(20); %Axis 2, minimum, maximum
                deg2rad(-100) deg2rad(144); %Axis 3, minimum, maximum
                deg2rad(-350) deg2rad(350); %Axis 4, minimum, maximum
                deg2rad(-120) deg2rad(120); %Axis 5, minimum, maximum
                deg2rad(-350) deg2rad(350)]; %Axis 6, minimum, maximum

%maximum absolute speed of each joint rad/s or m/s
robot.velmax = [deg2rad(90); %Axis 1, rad/s
                deg2rad(80); %Axis 2, rad/s
                deg2rad(75); %Axis 3, rad/s
                deg2rad(90); %Axis 4, rad/s
                deg2rad(83); %Axis 5, rad/s
                deg2rad(130)];%Axis 6, rad/s
...
robot.axis=[-0.1 0.4 -0.3 0.3 0 0.4];
%read graphics files
