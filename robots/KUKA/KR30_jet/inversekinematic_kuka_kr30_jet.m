...
function q = inversekinematic_kuka_kr30_jet(robot, T)

%initialize q,
%four possible solutions are feasible
q=zeros(6,4);
...
%first joint, one possible solution admited: 
q1=Pm(3);
...
%q(1,:) = normalize(q(1,:)); Tranlational, do not normalize to -pi, pi
q(2,:) = normalize(q(2,:));
...
%solve_for_theta3
q2(1) = pi/2 - beta - gamma+deg2rad(20.34); %elbow up
q2(2) = pi/2 - beta + gamma+deg2rad(20.34); %elbow down
