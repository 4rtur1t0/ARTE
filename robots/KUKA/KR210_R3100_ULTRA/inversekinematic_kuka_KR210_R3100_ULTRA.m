...
%   Solves the inverse kinematic problem for the KUKA KR210 R3100 ULTRA robot
%   where:
%   robot stores the robot parameters.
%   T is an homogeneous transform that specifies the position/orientation
%   of the end effector.
%
%   A call to Q=INVERSEKINEMATIC__KUKA_KR210_R3100_ULTRA returns 8 possible solutions, thus,
%   Q is a 6x8 matrix where each column stores 6 feasible joint values.
%
%   
%   Example code:
%
%   robot=load_robot('kuka', 'KR210_R3100_ULTRA');
%   q = [0 0 0 0 0 0];	
...
function q = inversekinematic_kuka_KR210_R3100_ULTRA(robot, T)
...
gamma = real(acos((L2^2+r^2-Lx^2)/(2*r*L2)));
...
phi=acos((A2^2+Lx^2-L3^2)/(2*A2*Lx));
...
eta = real(acos((L2^2 + Lx^2 - r^2)/(2*L2*Lx)));
