%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Q = INVERSEKINEMATIC_motomanMA2010(robot, T)	
%   Solves the inverse kinematic problem for the YASKAWA MOTOMAN MA2010 robot
%   where:
%   robot stores the robot parameters.
%   T is an homogeneous transform that specifies the position/orientation
%   of the end effector.
%
%   A call to Q=INVERSEKINEMATIC_YASKAWA_MOTOMAN returns 8 possible solutions, thus,
%   Q is a 6x8 matrix where each column stores 6 feasible joint values.
%
%   
%   Example code:
%
%   YASKAWA=load_robot('YASKAWA', 'Motoman_MA2010');
%   q = [0 0 0 0 0 0];	
%   T = directkinematic(YASKAWA, q);
%   %Call the inversekinematic for this robot
%   qinv = inversekinematic(YASKAWA, T);
%   check that all of them are feasible solutions!
%   and every Ti equals T
%   for i=1:8,
%        Ti = directkinematic(YASKAWA, qinv(:,i))
%   end
...
% Copyright (C) 2012, by Saúl Fernández Candela y David García Egío
%
...
function q = inversekinematic_motomanMA2010(robot, T)
...
if ~isreal(gamma)
    disp('WARNING:inversekinematic_YASKAWA_MOTOMAN_MA2010: the point is not reachable for this configuration, imaginary solutions'); 
    %gamma = real(gamma);
end
