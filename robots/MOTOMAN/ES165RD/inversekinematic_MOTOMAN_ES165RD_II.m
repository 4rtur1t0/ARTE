...
function q = inversekinematic_MOTOMAN_ES165RD_II(robot,T)
%initialize q,
...
r = sqrt(p1(1)^2 + p1(2)^2);
phi=real(acos((L2^2-r^2+L3prima^2)/(2*L2*L3prima)));
phi2=acos((L2^2-r^2+L3prima^2)/(2*L2*L3prima));
%given q1 is known, compute first DH transformation
T01=dh(robot, q, 1);

beta2 = real(acos((L2^2 + L3prima^2 - r^2)/(2*L2*L3prima)));
beta3=acos((L2^2 + L3prima^2 - r^2)/(2*L2*L3prima));
beta = atan2(p1(2), p1(1));
