...
function q = inverse_kinematics_sawyer2(robot, Tf, q0)
%global parameters
...
%     end
    ...
    manip = sqrt(det(J*J'))
    % consider that the torques are directly translated to joint speeds
    qd = J'*Vref;
...
v = [0.01 1  10  10  20 20  20  20];
kp = interp1(x,v,e, 'linear','extrap');
% finally scale
qd = kp*qd;
