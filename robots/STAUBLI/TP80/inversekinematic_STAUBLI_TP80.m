%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Q = INVERSEKINEMATIC_STAUBLI_TP80(robot, T)	
%   Solves the inverse kinematic problem for the STAUBLI TP80 robot
%   where:
%   robot stores the robot parameters.
%   T is an homogeneous transform that specifies the position/orientation
%   of the end effector.
%
%   A call to Q=INVERSEKINEMATIC_STAUBLI_TP80 returns 4 possible solutions, thus,
%   Q is a 4x4 matrix where each column stores 6 feasible joint values.
%
%   
%   Example code:
%
%   robot=load_robot('STAUBLI', 'TP80');
%   q = [0 0 0 0];	
...
function q = inversekinematic_STAUBLI_TP80(robot, T)%SOLO HEMOS CAMBIADO EL NOMBRE
...
theta = eval(robot.DH.theta);%en el robot de la libreria ejemplo no está
d = eval(robot.DH.d);
a = eval(robot.DH.a);
alpha = eval(robot.DH.alpha);%en el robot de la libreria ejemplo no está
...
shift = d(3);%en el robot de la libreria ejemplo no está
...
v=[Px Py Pz 1]';%expresado en el sistema de referencia de la base en la pared
