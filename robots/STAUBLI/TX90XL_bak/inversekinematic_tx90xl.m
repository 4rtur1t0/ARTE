...
function q = inversekinematic_tx90xl(robot, T)
...
if ~isreal(gamma)
    disp('WARNING:inversekinematic_tx90xl: the point is not reachable for this configuration, imaginary solutions'); 
    %gamma = real(gamma);
end
...
% % %q1=atan2(Pm(2), Pm(1));
% % q1=acos((2*0.05^2)/(2*0.05*sqrt(Pm(1)^2+Pm(2)^2))-pi+acos(Pm(1)/(sqrt(Pm(1)^2+Pm(2)^2));
% % 
% % 
% % %solve for q2
% % q2_1=solve_for_theta2(robot, [q1 0 0 0 0 0 0], Pm);
...
% % % for any of the possible combinations (theta1, theta2, theta3)
% % for i=1:2:size(q,2),
% %     qtemp = solve_spherical_wrist_rx160l(robot, q(:,i), T, 1); %wrist up
% %     qtemp(4:6)=normalize(qtemp(4:6));
% %     q(:,i)=qtemp;
% %     
% %     qtemp = solve_spherical_wrist_rx160l(robot, q(:,i), T, -1); %wrist down
% %     qtemp(4:6)=normalize(qtemp(4:6));
% %     q(:,i+1)=qtemp;
% % end
% % 
...
% % 
% % if ~isreal(gamma)
% %     disp('WARNING:inversekinematic_rx160l: the point is not reachable for this configuration, imaginary solutions'); 
% %     %gamma = real(gamma);
% % end
% % 
% % %return two possible solutions
...
% % % For wrists with the same orientation as in the KUKA KR30_jet
% % %--> use solve_spherical_wrist
% % function q = solve_spherical_wrist_rx160l(robot, q, T, wrist)
% % 
% % 
% % % T is the noa matrix defining the position/orientation of the end
% % % effector's reference system
