...
robot.name= 'STÄUBLI_TX200';

robot.DH.theta= '[q(1) q(2)-pi/2 q(3)-pi/2 q(4) q(5) q(6)+pi]';
robot.DH.d='[0.642 0 0 0.8 0 0.194]';
robot.DH.a='[0.257 0.950 0 0 0 0]';
robot.DH.alpha= '[-pi/2 0 -pi/2 pi/2 -pi/2 0]';
robot.J=[];


robot.inversekinematic_fn = 'inversekinematic_TX200(robot, T)';
...
robot.maxangle =[-pi pi; %Axis 1, minimum, maximum
                deg2rad(-120) deg2rad(120); %Axis 2, minimum, maximum
                deg2rad(-140) deg2rad(145); %Axis 3
                deg2rad(-270) deg2rad(270); %Axis 4: Unlimited (400ï¿½ default)
                deg2rad(-120) deg2rad(120); %Axis 5
                deg2rad(-270) deg2rad(270)]; %Axis 6: Really Unlimited to (800ï¿½ default)

%maximum absolute speed of each joint rad/s or m/s
robot.velmax = [deg2rad(160); %Axis 1, 
                deg2rad(160); %Axis 2, 
                deg2rad(160); %Axis 3, 
                deg2rad(260); %Axis 4, 
                deg2rad(260); %Axis 5, 
...
robot.axis=[-1.2 1.2 -1.2 1.2 0 3];
%read graphics files
