%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Q = INVERSEKINEMATIC_rx170bl(robot, T)	
...
function q = inversekinematic_rx170bl(robot, T)
...
for i=1:2:size(q,2),
    qtemp = solve_spherical_wrist_rx170bl(robot, q(:,i), T, 1); %wrist up
    qtemp(4:6)=normalize(qtemp(4:6));
    q(:,i)=qtemp;
    
    qtemp = solve_spherical_wrist_rx170bl(robot, q(:,i), T, -1); %wrist down
    qtemp(4:6)=normalize(qtemp(4:6));
    q(:,i+1)=qtemp;
end
...
if ~isreal(gamma)
    disp('WARNING:inversekinematic_rx170bl: the point is not reachable for this configuration, imaginary solutions'); 
    %gamma = real(gamma);
end
...
function q = solve_spherical_wrist_rx170bl(robot, q, T, wrist)
