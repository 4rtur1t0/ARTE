...
function [q_t, qd_t, qdd_t, time] = compute_trapezoidal_profile(q_initial, qd_initial, q_final, qd_final, qdmax, taccel)
    ...
    [qd_max, total_time]=synchronize_movement(q_initial, qd_initial, q_final, qd_final, qdmax, taccel);
    % find max time and replan with that
    [q_t, qd_t, qdd_t, time]=trapezoidal(q_initial, qd_initial, q_final, qd_final, qd_max, taccel, total_time);
end



% Compute minimum time to complete movement
function [qd_max, max_time]=synchronize_movement(q_initial, qd_initial, q_final, qd_final, qdmax, taccel)
    ...
    dq_total = q_final-q_initial;
    dqII = dq_total -dqI -dqIII;

    % T should be positive for a trapezoidal profile
    % if T is negative for any joint, then a replanning must be done to
    ...
    % recompute the max speed for the constant speed segment
    qd_max = (dq_total - dqI - dqIII)/max_time_constant;    
    max_time = max_time_constant + 2*taccel;
end
...
function [q_t, qd_t, qdd_t, time] = second_segment(q_initial, qd_initial, time_start, time_constant_speed)
    ...
    time = 0:delta_time:(time_constant_speed-delta_time);
    % offset time
    time = time + time_start + delta_time;
