%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  [ttotal, traj_type, qt, qdt, qddt, time]=trapezoidal(q0, qf, omega, 
%                                           alpha, delta_time, case_type)
%
...
%   interpolation.
%   
%   With two possible uses/behaviours:
%   A) Given initial and end values q0, qf, speed, acceleration, delta_time and
%   case_type='plan_time'. It returns the total time, type of trajectory
%   (trapezoidal or triangular), qt, qdt, qddt and a vector of times.
%
%   B) Given initial and end values q0, qf, total time, acceleration, delta_time and
%   case_type='plan_speed'. It returns the total time, type of trajectory
%   (trapezoidal or triangular), qt, qdt, qddt and a vector of times.
%   Internally, the speed needed to achieve the movement is computed.
%
...
function [ttotal, traj_type, qt, qdt, qddt, time ]=trapezoidal(s0, sf, speed, accel, delta_time, case_type)

if (case_type== 'caseA') || (case_type== 'plan_time')
    [ttotal, traj_type, qt, qdt, qddt, time]=caseA(s0, sf, speed, accel, delta_time);
    
else
    % in this case, the third parameter is the total time, and not the
    % speed
    ttotal=speed;
    % this case 
    traj_type='trapezoidal';    
    [qt, qdt, qddt, time, traj_type]=caseB(s0, sf, ttotal, accel, delta_time);
end
...
function [qt, qdt, qddt, time, traj_type]=caseB(q0, qf, ttotal, alpha, delta_time)
