...
function simulation = coppelia_init(n_joints, n_col_objects)
    %global simulation
    simulation = [];
    simulation.n_joints = n_joints;
    simulation.n_collision_objects = n_col_objects;
    simulation.joint_handles = []; 
    simulation.collision_handles = [];
    disp('Program started');
    % sim=remApi('remoteApi','extApi.h'); % using the header (requires a compiler)
    sim=remApi('remoteApi'); % using the prototype file (remoteApiProto.m)
    sim.simxFinish(-1); % just in case, close all opened connections
    simulation.sim = sim;
    simulation.clientID=sim.simxStart('127.0.0.1',19997,true,true,5000,5);

    if (simulation.clientID>-1)
        disp('Connected to remote API server');
        % enable the synchronous mode on the client:
        sim.simxSynchronous(simulation.clientID,true);

        % start the simulation:
        sim.simxStartSimulation(simulation.clientID,sim.simx_opmode_blocking);
        j_handles = [];
        c_handles = [];
        
        for i=1:simulation.n_joints
            joint_name = strcat('q', int2str(i));
            [r, j_handles(i)]=sim.simxGetObjectHandle(simulation.clientID, joint_name, sim.simx_opmode_blocking);
        end
        ...
        simulation.joint_handles = j_handles;
        simulation.collision_handles = c_handles;
