function interception(objetivo,modo)


%       Script realizado por:
%		-Sonia Pavia alba
%		-Daniel Hernández Santamaría
...
switch modo
    case 'par'
        ...
        r=Rmin_cohete+(Rmax_cohete-Rmin_cohete)*rand;                      
        phi=2*pi*rand;
...
e_aprox= atan2((p_corte(3)-2),(sqrt(p_corte(1)^2+p_corte(2)^2)));

if p_corte(3)<h_canon
    e_aprox=-e_aprox;
end

acimut = atan2(norm(cross(ac_final,ac_inicial)),dot(ac_final,ac_inicial));
...
delta_t=(tfinal-(td_aprox-0.6))/step;             %Intervalos de tiempo y angulo para simulación
delta_e=(2*degtorad(5))/step;

%Bucles simulacion
for elevacion=(e_aprox-degtorad(5)):delta_e:(e_aprox+degtorad(5))
    for td=(td_aprox-0.6):delta_t:tfinal
       
       completado=(count/(2*step))*100;
       fprintf('\n\n\t\tCálculo completado al %d %\n\n',completado); 
...
%%%%%%%%%%%%%%% Simulacion %%%%%%%%%%%%%%%%%%%
        ...
        simul=sim('torreta','StopTime',num2str(tfinal), 'FixedStep', num2str(delta_t));
        ...
        for i=1:length(dis_col)                                         %El siguiente bucle anidado evalua si ha habido un impacto cumpliendo restricciones tanto articulares
            if dis_col(i)<precision                                     %como de precisión
                ...
                if ((theta_min<elevacion && elevacion<theta_max) && (tt<td))
                    ...
                    fprintf('\n Logrado con los siguientes parámetros de disparo:\n\t- Acimut: %d\n\t- Elevación: %d\n\t- Tiempo de disparo: %d\n\t- Distancia de colision: %d',acimut,elevacion,td,dis_col(i))
