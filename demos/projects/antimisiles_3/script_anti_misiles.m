...
v=1035; %Velocidad del antimisil en m/s.
...
if t3+20<t2 fprintf('Misil objetivo alcanzable\n');
else fprintf('Misil objetivo inalcanzable\n');
end

 ih=0;
 iv=0;
 while ih<beta || iv<=gamma
     if ih<beta
     ih=ih+0.1; %Aumentar el paso para mayor precisión pero menor velocidad de simulación
     end
     if iv<=gamma
     iv=iv+0.1;
     end
     drawrobot3d(robot,[ih iv]')
 end
 while ih>beta || iv<=gamma
      if ih>beta
     ih=ih-0.1;
 ...
 %El robot no se posicionará exactamente en q=[beta gamma] si el paso es
 %pequeño